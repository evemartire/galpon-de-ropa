jQuery(document).ready(function ($) {
    ...
    $('#notification-success-top > button').on('click', function () {
        $('#notification-success-top').hide(700);
        return false;
    });
    $('#notification-error-top > button').on('click', function () {
        $('#notification-error-top').hide(700);
        return false;
    });

    $('#closeErrors').click(function () {
        $('#notification-form-error').hide(700);
        return false;
    });
    setTimeout(resetNotificaciones, 3000);
});
...
function agregarNotificacion(tipo, mensaje) {
    var notificacion = '<div id="notification-' + tipo + '-top" class="notification notification-' + tipo + '-top notification-' + tipo + '-top-show col-xs-12">';
    notificacion += '<button aria-hidden="true" class="close" type="button">×</button>';
    notificacion += '<strong>' + mensaje + '!</strong>';
    notificacion += '</div>';
    $('#notificaciones').html(notificacion);

    setTimeout(resetNotificaciones, 5000);
}